...
extension HomeViewController: UITableViewDelegate, UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if let voteAverage = movie.voteAverage, voteAverage != 0.0 {
            ...
            cell.ratingLabel.text = Constants.Icon.rating + String(format: "%.1f", voteAverage)
        } else {
            cell.ratingLabel.isHidden = true
        }
        
        cell.releaseDateLabel.text = Constants.Icon.releaseDate + (movie.releaseDate ?? "N/A")
        cell.posterImageView.sd_setImage(with: URL(string: Constants.API.posterUrl.appending(movie.posterPath ?? "")), completed: nil)
